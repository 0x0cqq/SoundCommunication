...
class PackageConfig: 
    def __init__(self, sampling_freq: float, amplitude: float, signal_duration: float, ):
...
class Packager:
    ...
    def unpackage(self, signal: np.ndarray) -> np.ndarray:
        # 先进行一个带通滤波
        b, a = sig.iirfilter(5, [5000, 15000], btype="bandpass", fs=self.modulator.config.sampling_freq)
        ...
        len_data = self.modulator.demodulate(signal[start : start + single_signal_len * 8])
        print(len_data)
        ...
        data = self.modulator.demodulate(signal[start:start+single_signal_len*data_len])
